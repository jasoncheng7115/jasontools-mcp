...
22 tools, compact responses, human-readable parameters.
...
def list_devices(limit: int = 50, status: Optional[str] = None,
    ...
    """List/search LibreNMS devices (paginated, default 50).
    ...
    Args:
        limit: Max devices returned (default 50, 0=all).
...
def get_device_sla(device: Optional[str] = None, days: int = 30, limit: int = 10) -> str:
    ...
    try:
        if device:
            ...
            return _R({
                "device": {"device_id": device_id, "hostname": dev.get("hostname"),
                           "sysName": dev.get("sysName"), "status": dev.get("status")},
                "availability": avail,
                "outages": outages,
            ...
            for d in devices:
                ...
                try:
                    result = _api_request("GET", f"devices/{did}/availability")
                    avail = _extract_data(result, ['availability'])
                except Exception:
                    avail = []
            ...
            return _R({"data": rankings[:limit], "count": min(limit, len(rankings)),
                        "period_days": days})
    except Exception as e:
        return _R({"error": str(e)})
...
def get_cisco_sla(device: Optional[str] = None, limit: int = 50) -> str:
    ...
    try:
        ...
        return _R({"data": data, "count": len(data)})
    except Exception as e:
        return _R({"error": str(e)})
