...
- Manual Starlette setup does not include DNS rebinding middleware (no 421 issue)

Changes in 1.9.39:
- Added API key authentication middleware (--api-key / MCP_API_KEY)
- Refactored SSE/streamable-http transport to share common middleware setup

Changes in 1.9.38:
- Added SSE transport support (--transport sse)
- Fixed false-positive API error detection: legitimate empty query results no longer trigger error message
- Error response only triggers when actual API exceptions occur (network/auth failures)
...
async def main():
    ...
    try:
        ...
        if config['help']:
            ...
            print()
            print("New in v1.9.40:")
            print("  [OK] SSE transport with API key auth (--transport sse --api-key)")
            print("  [OK] No DNS rebinding issue (manual Starlette, no 421 error)")
            ...
            print("  MCP_TRANSPORT - Transport mode: stdio, streamable-http, or sse (default: stdio)")
            print("  MCP_HTTP_HOST - HTTP listen host (default: 0.0.0.0)")
            print("  MCP_HTTP_PORT - HTTP listen port (default: 8000)")
            print("  MCP_API_KEY   - API key for SSE/HTTP authentication (optional)")
            print()
            ...
            print("  --transport <mode>  - Transport mode: stdio (default), streamable-http, or sse")
            ...
            print()
            print("  # Start with SSE transport:")
            print("  python3 mcp_graylog.py --transport sse --http-port 8001")
            return
        ...
        if transport == "stdio":
            ...
            # doesn't include TransportSecurityMiddleware (only FastMCP adds it)

            print(f"{transport.upper()} server listening on http://{http_host}:{http_port}{endpoint_path}", file=sys.stderr)
