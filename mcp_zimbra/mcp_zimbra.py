...
- v1.9.0 (2026-02-26): ENHANCEMENT - searchByAttribute extra_attrs parameter
                       - New param `extra_attrs`: comma-separated attribute names to include
                         beyond default fields (saves tokens; only returns what you ask for)
- v1.8.9 (2026-02-13): BUG FIX - Accurate account counting via pagination
                       - getAccountCount: fixed undefined variable crash + wrong counts
                         (Zimbra doesn't return searchTotal; now uses paginated element counting)
                       - countAccountByCOS: same fix — paginated counting instead of limit=1
                         (was returning 0 for all COS due to missing searchTotal attribute)
- v1.8.8 (2026-02-13): OPTIMIZATION - Server-side filtering audit
                       - countAccountByCOS: rewritten to use LDAP per-COS query
                         (was fetching ALL accounts into Python to count — 5-50MB waste)
                       - getAllAccounts: status_filter now uses server-side LDAP query
                         (was client-side Python loop — pagination/total were inaccurate)
- v1.8.7 (2026-02-13): ENHANCEMENT - getAccountCount server-side filtering
                       - Rewritten to use SearchDirectoryRequest with LDAP filter (was client-side)
                       - NEW PARAM: query — arbitrary LDAP filter for advanced filtering
                       - status_filter + query auto-combined with AND
                       - Drastically reduces token usage (returns count only, not full account list)
- v1.8.6 (2026-02-13): FEATURE - Added SSE transport support
                       - --transport now accepts: stdio (default), sse, streamable-http
                       - SSE transport uses same --host/--port as streamable-http
                       - Backed by mcp SDK 1.22.0 native run_sse_async()
- v1.8.5 (2026-02-13): FEATURE - Mail read tools gated by ZIMBRA_ENABLE_MAIL_READ toggle
                       - searchMail, getMailDetail, getConversation, listFolders, getMailAttachment
                         only registered when ZIMBRA_ENABLE_MAIL_READ=true (default: false)
                       - NEW DECORATOR: mail_read_tool() — conditional tool registration
                       - Startup log shows DISABLED hint when mail read is off
...
1. Command Line Arguments (Recommended):
   ...
   --transport             Transport mode: stdio (default), sse, or streamable-http
...
class ZimbraConfig:
    ...
    def __init__(self, cli_args=None):
        ...
        self.enable_mail_read = self._get_bool_value(cli_args, 'enable_mail_read', 'ZIMBRA_ENABLE_MAIL_READ', False)
...
if len(sys.argv) > 1:
    def _parse_args():
        ...
        parser.add_argument('--enable-mail-read', type=lambda x: x.lower() in ('true', '1', 'yes'), help='Enable mail read tools (default: false)')
...
def getAllAccounts(domain: Optional[str] = None,
    ...
    try:
        ...
        # Server-side LDAP filter for status
        query_element = ""
        if status_filter:
            query_element = f'<query><![CDATA[(zimbraAccountStatus={status_filter})]]></query>'
...
def getAccountCount(domain: Optional[str] = None,
    ...
    """Count accounts with server-side filtering by status, LDAP query, or domain.

    Args:
        domain: Filter by domain. Example: "example.com" or "a.com,b.com". Default: all domains.
        status_filter: Filter by account status (shortcut). Values: "active", "closed", "locked", "lockout", "maintenance", "pending". Default: all.
        query: LDAP filter for advanced filtering. Example: "(zimbraMailHost=mail1.example.com)", "(zimbraCOSId=UUID)". Combines with status_filter using AND. Default: none.
    """
    logger.info(f"Getting account count (domain={domain}, status={status_filter}, query={query})")

    try:
        # Build LDAP filter from parameters
        ldap_parts = []
        if status_filter:
            ldap_parts.append(f"(zimbraAccountStatus={status_filter})")
        if query:
            ldap_parts.append(query)

        if len(ldap_parts) > 1:
            ldap_query = f"(&{''.join(ldap_parts)})"
        elif len(ldap_parts) == 1:
            ldap_query = ldap_parts[0]
        else:
            ldap_query = ""

        domain_query = ""
        if domain:
            for d in [x.strip() for x in domain.split(',')]:
                domain_query += f'<domain by="name">{d}</domain>'

        query_element = f"<query><![CDATA[{ldap_query}]]></query>" if ldap_query else ""

        # Paginated server-side count: LDAP filter on server, count elements
        ns = {'zimbra': 'urn:zimbraAdmin'}
        total_matches = 0
        offset = 0
        page_size = 1000
        ...
        while True:
            ...
            # Try searchTotal first (if Zimbra returns it, use it directly)
            if search_response is not None and search_response.get('searchTotal'):
                total_matches = int(search_response.get('searchTotal'))
                break

            # Otherwise count returned elements and paginate
            returned = len(root.findall('.//zimbra:account', ns))
            total_matches += returned

            has_more = search_response is not None and search_response.get('more') == '1'
        ...
        if status_filter:
            filters_applied["account_status"] = status_filter
        if query:
            filters_applied["ldap_query"] = query
...
def searchByAttribute(query: str,
                      ...
                      limit: int = 500,
                      offset: int = 0,
                      extra_attrs: Optional[str] = None) -> str:
    ...
    Args:
        ...
        domain: Filter by domain. Example: "example.com". Default: all domains.
        extra_attrs: Comma-separated attribute names to include in results beyond the defaults. Example: "zimbraNotes,zimbraMailQuota,description". Default: none (only common fields returned).
    ...
    try:
        ...
        # Parse requested extra attributes
        extra_set = set()
        if extra_attrs:
            extra_set = {a.strip() for a in extra_attrs.split(',') if a.strip()}
        ...
        for elem in root.findall('.//zimbra:account', ns):
            ...
            for attr_name in extra_set:
                if attr_name in attrs:
                    entry[attr_name] = attrs[attr_name]
...
def countAccountByCOS(cos_name: Optional[str] = None) -> str:
    ...
    try:
        ...
        for cos_elem in cos_elems:
            cos_map[cos_elem.get('id')] = cos_elem.get('name')

        # Step 2: Count accounts per COS using paginated element counting
        cos_statistics = []
        total_accounts = 0
        ns = {'zimbra': 'urn:zimbraAdmin'}
        page_size = 1000
        ...
        for cos_id, cos_name_val in cos_map.items():
            ...
            while True:
                ...
                <SearchDirectoryRequest xmlns="urn:zimbraAdmin"
                                        ...
                                        offset="{offset}"
                                        types="accounts">
                    <query><![CDATA[(zimbraCOSId={cos_id})]]></query>
                </SearchDirectoryRequest>
                """

                count_root = query_zimbra_api(count_soap, enable_cache=False)
                parse_zimbra_response(count_root)

                search_resp = count_root.find('.//{urn:zimbraAdmin}SearchDirectoryResponse')

                # Try searchTotal first (if Zimbra returns it)
                if search_resp is not None and search_resp.get('searchTotal'):
                    count = int(search_resp.get('searchTotal'))
                    break

                # Otherwise count returned elements and paginate
                returned = len(count_root.findall('.//zimbra:account', ns))
                count += returned

                has_more = search_resp is not None and search_resp.get('more') == '1'
                if not has_more or returned == 0:
                    break
                offset += returned
...
Usage Examples:
  ...
  Environment Variables:
    ...
    feature_group.add_argument('--enable-mail-read', type=lambda x: x.lower() in ('true', '1', 'yes'),
                               help='Enable mail read tools: searchMail, getMailDetail, getConversation, listFolders, getMailAttachment (default: false)')
    ...
    transport_group.add_argument('--api-key', '-k',
                                default=os.environ.get('MCP_API_KEY', ''),
                                help='API key for SSE/HTTP authentication (or set MCP_API_KEY env var)')
...
if __name__ == "__main__":
    ...
    if zimbra_config.auth_mode == "user":
        ...
        tool_count = 44 + mail_read_count  # 44 base + 5 mail_read
        logger.info(f"Available Tools ({tool_count} total, admin mode):")
    ...
    api_key = getattr(cli_args, 'api_key', '') or ''
    ...
    logger.info(f"  Mail Read: {'enabled' if zimbra_config.enable_mail_read else 'disabled'}")
    logger.info(f"  Transport: {transport_mode}" +
                (f" ({http_host}:{http_port})" if transport_mode in ('sse', 'streamable-http') else ''))
    if api_key and transport_mode in ('sse', 'streamable-http'):
        logger.info(f"  API Key: enabled")
    logger.info("=" * 80)
