#!/usr/bin/env python3
"""
OPNsense MCP Server - Config.xml Based (Enhanced with Service Management and Firmware/Package Info)
...
License: MIT
Created: 2025-06-25
Updated: 2025-07-06 - Added firmware and package version information functionality

This MCP server provides OPNsense firewall rule reading capabilities
by parsing config.xml directly and service status monitoring, plus firmware/package management.

Enhanced Features:
- Service status monitoring
- Service search and listing
- Individual service status check
- Firmware version and status information
- Package and plugin version listing
- Firmware changelog access
- Package details and licensing information
"""
...
class OPNsenseClient:
    ...
    # === Firmware Management (Read-only) ===
    
    async def get_firmware_status(self) -> Dict:
        """Get firmware status and update information"""
        try:
            return await self._request("GET", "/api/core/firmware/status")
        except Exception as e:
            logger.error(f"Firmware status failed: {str(e)}")
            raise
    
    async def get_firmware_info(self) -> Dict:
        """Get comprehensive firmware information including packages and plugins"""
        try:
            return await self._request("GET", "/api/core/firmware/info")
        except Exception as e:
            logger.error(f"Firmware info failed: {str(e)}")
            raise
    ...
    async def get_firmware_check(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"Firmware check failed: {str(e)}")
            raise
    
    async def get_firmware_changelog(self, version: str = None) -> Dict:
        """Get firmware changelog for specific version or latest"""
        try:
            if version:
    ...
    async def get_package_details(self, package_name: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Package details failed for {package_name}: {str(e)}")
            raise
    ...
    async def get_package_license(self, package_name: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Package license failed for {package_name}: {str(e)}")
            raise
    
    async def get_firmware_comprehensive_overview(self) -> Dict:
        """Get comprehensive firmware and package overview"""
        try:
            overview = {
                "timestamp": datetime.now().isoformat(),
                "overview_type": "firmware_comprehensive"
            }
            
            # Get firmware status
            try:
                overview["firmware_status"] = await self.get_firmware_status()
            except Exception as e:
                overview["firmware_status_error"] = str(e)
            
            # Get firmware info (includes packages and plugins)
            try:
                overview["firmware_info"] = await self.get_firmware_info()
            except Exception as e:
                overview["firmware_info_error"] = str(e)
            
            # Get firmware settings
            try:
                overview["firmware_settings"] = await self.get_firmware_settings()
            except Exception as e:
                overview["firmware_settings_error"] = str(e)
            
            # Get firmware options
            try:
                overview["firmware_options"] = await self.get_firmware_options()
            except Exception as e:
                overview["firmware_options_error"] = str(e)
            
            # Get firmware health
            try:
                overview["firmware_health"] = await self.get_firmware_health()
            except Exception as e:
                overview["firmware_health_error"] = str(e)
            
            # Get firmware audit
            try:
                overview["firmware_audit"] = await self.get_firmware_audit()
            except Exception as e:
                overview["firmware_audit_error"] = str(e)
            
            # Get firmware connection
            try:
                overview["firmware_connection"] = await self.get_firmware_connection()
            except Exception as e:
                overview["firmware_connection_error"] = str(e)
            
            return overview
            
        except Exception as e:
            logger.error(f"Firmware comprehensive overview failed: {str(e)}")
            raise
...
async def handle_list_tools() -> List[Tool]:
    ...
    return [
        ...
        # === Firmware and Package Management Tools ===
        Tool(
            name="get_firmware_status",
            description="Get firmware status and update information",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            ...
            description="Get comprehensive firmware information including packages and plugins",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            name="get_firmware_changelog",
            description="Get firmware changelog for specific version or latest",
            inputSchema={
                "type": "object",
                "properties": {
                    "version": {
        ...
        Tool(
            name="get_firmware_audit",
            description="Get firmware security audit information",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            name="get_firmware_health",
            description="Get firmware health check information",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            name="get_firmware_connection",
            description="Get firmware repository connectivity information",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            name="get_firmware_options",
            description="Get firmware mirror and flavour options",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            ...
            description="Get license information for a specific package",
            inputSchema={
                "type": "object",
                "properties": {
                    "package_name": {
                        "type": "string",
                        "description": "Name of the package to get license for"
                    }
                },
                "required": ["package_name"]
            }
        ...
        Tool(
            name="get_firmware_comprehensive_overview",
            description="Get comprehensive firmware and package overview with all available information",
            inputSchema={
                "type": "object",
                "properties": {}
            }
...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        async with OPNsenseClient() as client:
            ...
            # === Firmware and Package Management Tools ===
            elif name == "get_firmware_status":
                result = await client.get_firmware_status()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Status:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_info":
                result = await client.get_firmware_info()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Information (包含套件與外掛程式):\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_check":
                result = await client.get_firmware_check()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Update Check:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_changelog":
                version = arguments.get("version")
                result = await client.get_firmware_changelog(version)
                
                version_text = f" for version {version}" if version else ""
                return [TextContent(
                    type="text",
                    text=f"OPNsense Firmware Changelog{version_text}:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_audit":
                result = await client.get_firmware_audit()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Security Audit:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_health":
                result = await client.get_firmware_health()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Health Check:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_connection":
                result = await client.get_firmware_connection()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Repository Connectivity:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_options":
                result = await client.get_firmware_options()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Mirror and Flavour Options:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_settings":
                result = await client.get_firmware_settings()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Configuration Settings:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_package_details":
                package_name = arguments["package_name"]
                result = await client.get_package_details(package_name)
                
                return [TextContent(
                    type="text",
                    text=f"Package Details for '{package_name}':\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_package_license":
                package_name = arguments["package_name"]
                result = await client.get_package_license(package_name)
                
                return [TextContent(
                    type="text",
                    text=f"Package License for '{package_name}':\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
            
            elif name == "get_firmware_comprehensive_overview":
                result = await client.get_firmware_comprehensive_overview()
                
                return [TextContent(
                    type="text",
                    text="OPNsense Firmware Comprehensive Overview:\n\n" + 
                         json.dumps(result, indent=2, ensure_ascii=False)
                )]
