...
- Detail views use field allowlists (DETAIL_*) to omit noise fields
- List tables truncate long values (30 chars) and use compact pipe separator
- Search results capped at 50 to prevent context overflow
- Transport: stdio, sse, streamable-http (via --transport flag)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="phpIPAM MCP Server v2.5")
    parser.add_argument(
        "--transport", "-t",
    ...
    parser.add_argument(
        ...
        help="Host for sse/streamable-http (default: 127.0.0.1)",
    )
    parser.add_argument(
    ...
    logger.info("=" * 60)
    logger.info("phpIPAM FastMCP Server v2.5")
    logger.info(f"  URL       : {config.BASE_URL}/api/{config.APP_ID}")
    logger.info(f"  Transport : {args.transport}")
    if args.transport in ("sse", "streamable-http"):
        logger.info(f"  Listen    : http://{args.host}:{args.port}")
        if args.api_key:
            logger.info("  Auth      : API key enabled")
        else:
            logger.info("  Auth      : none (no --api-key)")
    logger.info("=" * 60)
    ...
    if args.transport == "stdio":
        ...
        # NOTE: BaseHTTPMiddleware breaks SSE streaming (AssertionError on
        #       'http.response.start' inside body_stream), so we use a raw
        #       ASGI wrapper instead.
        if args.api_key:
            ...
            async def _auth_asgi_app(scope, receive, send):
                if scope["type"] in ("http", "websocket"):
                    ...
                    if token != _api_key:
                        # Return 401 without touching the inner app
                        await send({
                        ...
                        body = _json.dumps({"error": "Unauthorized", "message": "Invalid or missing API key"}).encode()
                        await send({
