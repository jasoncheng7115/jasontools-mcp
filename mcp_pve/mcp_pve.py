...
v1.5.4 (2026-02-26) - Fix: SSE/Streamable-HTTP 421 Misdirected Request for external clients
         ...
         - Added --api-key / MCP_API_KEY for optional Bearer token authentication
         - Fixes Chatbox and Open WebUI SSE connection errors

v1.5.3 (2026-02-24) - Enhancement: Add SSE transport and usage metrics in list tools
         - New --transport sse option for legacy MCP clients that don't support streamable-http
         - Uses mcp.server.sse.SseServerTransport with Starlette/Uvicorn
         - SSE endpoint: /sse, message endpoint: /messages/
         - Shares --http-host and --http-port settings with streamable-http
         - list_vms: Added cpu_usage_pct, memory_used_mb, netin, netout to summary output
         - list_containers: Added cpu_usage_pct, memory_used_mb, disk_used_gb, swap_used_mb, netin, netout
         - Tool descriptions updated with new field names for LLM discoverability

v1.5.2 (2026-02-23) - Fix: Remove schema maximum constraints on pagination parameters
         - Removed "maximum" from limit/limit_per_vm/limit_per_source in tool schemas (6 places)
         - mcpo OpenAPI validation was rejecting requests before code could clamp values
         - Internal validate_pagination_params() still enforces max limits safely
         - Fixes HTTP 500 when LLM sends limit exceeding schema maximum (e.g. limit=1000)
         - Moved IP fetching outside summary_only block so IP works in both modes

v1.5.1 (2026-02-23) - Enhancement: Add IP address to list_vms and list_containers
         - list_vms: Fetch VM IP via guest agent (parallel async calls)
         - list_containers: Fetch container IP via lxc interfaces API
         - Only queries running VMs/CTs; stopped ones return empty IP
         - Graceful fallback when guest agent is not installed
    ...
    all_tools = [
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "limit": {"type": "integer", "description": "Maximum number of items to return (default: 100, max: 500)", "default": 100, "minimum": 1},
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "limit_per_vm": {"type": "integer", "description": "Limit number of history entries per VM/CT", "default": 50, "minimum": 1}
                },
                "additionalProperties": False
            }
        ),
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "limit_per_source": {"type": "integer", "description": "Limit number of log entries per source (default: 100, max: 1000)", "default": 100, "minimum": 1},
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "node": {"type": "string", "description": "Node name"},
                    "limit": {"type": "integer", "description": "Limit number of tasks returned (default: 50, max: 1000)", "default": 50, "minimum": 1}
                },
                "required": ["node"],
                "additionalProperties": False
            }
        ...
        Tool(
            name="list_vms",
            description="List virtual machines with optional filtering and pagination. Returns vmid, name, status, node, cpus, cpu_usage_pct, memory_mb, memory_used_mb, disk_gb, netin, netout, uptime, ip. Use filter_name, filter_id, or filter_status to narrow results.",
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "limit": {"type": "integer", "description": "Maximum number of VMs to return (default: 100, max: 500)", "default": 100, "minimum": 1},
        ...
        Tool(
            ...
            description="List LXC containers with optional filtering and pagination. Returns vmid, name, status, node, cpus, cpu_usage_pct, memory_mb, memory_used_mb, disk_gb, disk_used_gb, swap_mb, swap_used_mb, netin, netout, uptime, ip. Use filter_name, filter_id, or filter_status to narrow results.",
            inputSchema={
                "type": "object",
                "properties": {
                    "node": {"type": "string", "description": "Node name (optional - lists from all nodes if not provided)"},
                    "summary_only": {"type": "boolean", "description": "Return only essential info to prevent overflow", "default": True},
                    "limit": {"type": "integer", "description": "Maximum number of containers to return (default: 100, max: 500)", "default": 100, "minimum": 1},
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "node": {"type": "string", "description": "Node name"},
                    "limit": {"type": "integer", "description": "Maximum number of log entries to return (default: 100, max: 1000)", "default": 100, "minimum": 1},
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "limit": {"type": "integer", "description": "Maximum number of entries to return (default: 100, max: 1000)", "default": 100, "minimum": 1},
...
async def execute_tool(name: str, arguments: dict) -> Union[dict, str]:
    ...
    async with client:
        ...
        # Virtual Machine Management
            ...
            # Fetch IPs in parallel for running VMs (requires guest agent)
            async def get_vm_ip(vm_node, vmid):
                try:
                    result = await client.get(f"/nodes/{vm_node}/qemu/{vmid}/agent/network-get-interfaces")
            ...
            for vm in paginated_vms:
                vm["ip"] = ip_results.get(vm.get("vmid"), "")
            ...
            if summary_only:
                ...
                for vm in paginated_vms:
                    ...
                    summary_vms.append({
                        ...
                        "node": vm.get("node"),
                        "cpus": vm.get("cpus", vm.get("maxcpu", 0)),
                        "cpu_usage_pct": round(cpu_usage * 100, 1) if cpu_usage else 0,
                        "memory_mb": round(maxmem / 1048576) if maxmem else 0,
                        "memory_used_mb": round(mem_used / 1048576) if mem_used else 0,
                        "disk_gb": round(maxdisk / 1073741824, 1) if maxdisk else 0,
        ...
        # Container Management
            ...
            # Fetch IPs in parallel for running containers
            async def get_ct_ip(ct_node, vmid):
                try:
                    result = await client.get(f"/nodes/{ct_node}/lxc/{vmid}/interfaces")
            ...
            for container in paginated_containers:
                if container.get("status") == "running":
                    ct_ip_tasks[container.get("vmid")] = get_ct_ip(container.get("node"), container.get("vmid"))

            ct_ip_results = {}
            if ct_ip_tasks:
                results = await asyncio.gather(*ct_ip_tasks.values(), return_exceptions=True)
                for vmid, result in zip(ct_ip_tasks.keys(), results):
                    ct_ip_results[vmid] = result if isinstance(result, str) else ""

            # Inject IP into each container dict
            for container in paginated_containers:
                container["ip"] = ct_ip_results.get(container.get("vmid"), "")
            ...
            if summary_only:
                ...
                for container in paginated_containers:
                    ...
                    summary_containers.append({
                        ...
                        "cpus": container.get("cpus", container.get("maxcpu", 0)),
                        "cpu_usage_pct": round(cpu_usage * 100, 1) if cpu_usage else 0,
                        "memory_mb": round(maxmem / 1048576) if maxmem else 0,
                        "memory_used_mb": round(mem_used / 1048576) if mem_used else 0,
                        "disk_gb": round(maxdisk / 1073741824, 1) if maxdisk else 0,
                        "disk_used_gb": round(disk_used / 1073741824, 1) if disk_used else 0,
                        "swap_mb": round(maxswap / 1048576) if maxswap else 0,
                        "swap_used_mb": round(swap_used / 1048576) if swap_used else 0,
...
async def main():
    ...
    try:
        ...
        if config['help']:
            ...
            print()
            print("Transport:")
            print("  --transport TYPE           Transport type: stdio | sse | streamable-http (env: MCP_TRANSPORT, default: stdio)")
            ...
            print("  --api-key KEY              API key for SSE/HTTP authentication (env: MCP_API_KEY)")
            print()
            print("Other:")
        ...
        if transport == "stdio":
            ...
            print(f"SSE server listening on http://{http_host}:{http_port}/sse", file=sys.stderr)
