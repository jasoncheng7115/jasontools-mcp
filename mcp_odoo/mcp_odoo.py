...
- Added API key authentication for HTTP transports (SSE/streamable-http)
  - Usage: python mcp_odoo.py --transport sse --api-key YOUR_SECRET_KEY
  - Or set MCP_API_KEY environment variable
  - Client must send: Authorization: Bearer YOUR_SECRET_KEY
  - Or: Authorization: YOUR_SECRET_KEY

NEW in v1.8.2:
- Disabled DNS rebinding protection for HTTP transports (SSE/streamable-http)
  - Fixes "421 Misdirected Request" error when connecting from external clients
  - Enables Chatbox, Open WebUI and other MCP clients to connect

NEW in v1.8.1:
- Fixed SSE and streamable-http transport to support custom host/port via uvicorn
  - Usage: python mcp_odoo.py --transport streamable-http --host 0.0.0.0 --port 8009
  - streamable-http endpoint: http://{host}:{port}/mcp
  - SSE endpoint: http://{host}:{port}/sse

NEW in v1.8.0:
- Added SSE (Server-Sent Events) transport support
  - Three transport modes: stdio, sse, streamable-http
- Added Markdown link field to search_partners for direct access to partner pages
...
def search_quotations(partner_name: Optional[str] = None, quotation_number: Optional[str] = None,
    ...
    Returns:
        ...
        Each quotation includes a 'link' field with ready-to-use Markdown link.

    IMPORTANT FOR DISPLAY: Use the 'link' field directly in your response.
        Example: Each result has link="[前往 S01234](URL)" - copy this directly.
        DO NOT create HTML <a> tags - they won't render.
    ...
    try:
        ...
        for quote in quotations:
            quote_url = _generate_quotation_url(quote['id'])
            quote_name = quote.get('name', f"ID:{quote['id']}")
            # Markdown link format for display
            markdown_link = f"[前往 {quote_name}]({quote_url})"
            ...
            if compact:
                enriched_quote = {
                    ...
                    'note': quote.get('note', ''),
                    'payment_term': quote.get('payment_term_id', [None, ''])[1] if quote.get('payment_term_id') else '',
                    'link': markdown_link
                }
            else:
                # Full mode
                enriched_quote = quote.copy()
                enriched_quote['link'] = markdown_link
...
def search_purchase_orders(partner_name: Optional[str] = None, po_number: Optional[str] = None,
    ...
    LINK FORMAT: When displaying links to user, use Markdown format:
        [採購單 P00123](URL) - DO NOT use HTML <a> tags.
    """
    ...
    try:
        ...
        for po in purchase_orders:
            po_url = _generate_purchase_order_url(po['id'])
            po_name = po.get('name', f"ID:{po['id']}")
            markdown_link = f"[前往 {po_name}]({po_url})"
            ...
            if compact:
                ...
                enriched_po = po.copy()
                enriched_po['odoo_url'] = po_url
                enriched_po['link'] = markdown_link
...
def search_delivery_orders(partner_name: Optional[str] = None, delivery_number: Optional[str] = None,
    ...
    LINK FORMAT: When displaying links to user, use Markdown format:
        [出貨單 WH/OUT/00123](URL) - DO NOT use HTML <a> tags.
    """
    ...
    try:
        ...
        for delivery in delivery_orders:
            delivery_url = _generate_delivery_order_url(delivery['id'])
            delivery_name = delivery.get('name', f"ID:{delivery['id']}")
            markdown_link = f"[前往 {delivery_name}]({delivery_url})"
            ...
            if compact:
                ...
                enriched_delivery['link'] = markdown_link
...
def search_products(name: Optional[str] = None, category_name: Optional[str] = None,
    ...
    LINK FORMAT: When displaying links to user, use Markdown format:
        [產品名稱](URL) - DO NOT use HTML <a> tags.
    """
...
def search_partners(name: Optional[str] = None, is_customer: Optional[bool] = None,
    ...
    LINK FORMAT: When displaying links to user, use Markdown format:
        [客戶名稱](URL) - DO NOT use HTML <a> tags.
    """
    ...
    try:
        ...
        if name:
            # Use OR to search in both name and display_name for better matching
            # Odoo ilike already adds % wildcards automatically
            domain.extend(['|', ['name', 'ilike', name], ['display_name', 'ilike', name]])
            logger.info(f"Partner search domain: {domain}")
        if email:
        ...
        search_kwargs = {'fields': partner_fields, 'order': 'name'}
        if limit > 0:
            # Increase internal limit for name search to get more results
            search_kwargs['limit'] = limit * 5 if name else limit
        if offset > 0:
            search_kwargs['offset'] = offset

        # Skip cache for name searches to ensure fresh results
        if name:
            partners = odoo.execute_kw('res.partner', 'search_read', [domain], search_kwargs)
        else:
            partners = _cached_odoo_call(
                'res.partner', 'search_read', [domain], search_kwargs
            )

        # Apply final limit after fetching
        if name and limit > 0:
        ...
        for partner in partners:
            partner_url = _generate_partner_url(partner['id'])
            partner_name = partner.get('name', f"ID:{partner['id']}")
            markdown_link = f"[前往 {partner_name}]({partner_url})"
            ...
            enriched_partner['link'] = markdown_link
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Odoo MCP Server - supports stdio, sse, and streamable-http transports',
...
Examples:
  ...
  python mcp_odoo.py --transport sse --port 8001
  ...
  python mcp_odoo.py --transport streamable-http --host 0.0.0.0 --port 9000
    ...
    if args.transport == 'stdio':
        ...
        if args.api_key:
            ...
            logger.info(f"API Key authentication: DISABLED (use --api-key to enable)")
        logger.info("Ready to serve MCP requests!")
        logger.info("=" * 80)
        # Use uvicorn to run with custom host/port
        import uvicorn
