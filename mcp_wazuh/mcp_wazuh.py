#!/usr/bin/env python3
"""
MCP server for Wazuh SIEM API - v1.3.5
===============================================================================
...
Changelog:
  v1.3.5 - Fix SSE/Streamable-HTTP for external clients (Chatbox)
  ...
  v1.3.4 - Add listGroups tool & SSE transport
  ...
  v1.3.3 - Severity labels & custom field queries
  ...
  v1.3.2 - Schema simplification for gpt-oss:120b tool calling compatibility
  ...
  v1.3.1 - camelCase tool naming for gpt-oss:120b compatibility
...
def _severity_label(level: int) -> str:
    """Convert Wazuh alert level (0-15) to severity label."""
    if level >= 12:
        return "Critical"
    if level >= 8:
        return "High"
...
def setup_http_sessions():
    ...
    manager_http_session.headers.update({
        "Content-Type": "application/json",
        "User-Agent": "wazuh-mcp-server/1.3.5"
    })
...
@mcp_server.tool()
def alertSummary(max_results: int = 300,
                  offset: int = 0,
                  min_level: int = 0,
                  max_level: int = 15,
...
def agentsWithAlerts(min_level: int = 0,
    ...
    """[Wazuh SIEM] List agents ranked by alert count, with severity and most recent alert info.

    This is a Wazuh SIEM tool, NOT Graylog. Use this to quickly find which hosts have the most alerts.

    Args:
        min_level: Min alert level 0-15, default 0. Levels: 0-3=low, 4-7=medium, 8-11=high, 12-15=critical.
        time_range_hours: Hours to look back, default 24. Example: 72=3 days, 168=1 week.
...
def vulnerabilitySummary(agent_identifier: str,
                          ...
                          severity_filter: str = "",
                          cve_filter: str = "") -> str:
    """[Wazuh SIEM] Get vulnerability scan results (CVE list) for a specific agent.

    This is a Wazuh SIEM tool, NOT Graylog. Use listAgents first to get agent_id, then query vulnerabilities with this tool.
...
@mcp_server.tool()
def listAgents(max_results: int = 300, status_filter: str = "active",
                name_filter: str = "", ip_filter: str = "",
                group_filter: str = "", os_filter: str = "",
                version_filter: str = "") -> str:
    """[Wazuh SIEM] List all Wazuh monitored agents (hosts/servers).

    This is a Wazuh SIEM tool, NOT Graylog. Use this first to get agent_id, then query agent-specific details with other tools.
    ...
    Example:
        listAgents(status_filter="active")
    """
...
def clusterHealth() -> str:
    """[Wazuh SIEM] Check Wazuh cluster health (enabled, running, connected nodes).
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("Wazuh SIEM MCP Server v1.3.5")
    logger.info("=" * 80)
    logger.info("Reference: Inspired by mcp-server-wazuh (Rust) by Gianluca Brigandi")
    logger.info("=" * 80)
    logger.info("Available Tools (18 total, camelCase):")
